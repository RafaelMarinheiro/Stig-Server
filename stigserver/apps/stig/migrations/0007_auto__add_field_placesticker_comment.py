...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field sticker_list on 'Comment'
        db.delete_table(db.shorten_name(u'stig_comment_sticker_list'))

        # Adding field 'PlaceSticker.comment'
        db.add_column(u'stig_placesticker', 'comment',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['stig.Comment'], null=True),
                      keep_default=False)


    def backwards(self, orm):
        # Adding M2M table for field sticker_list on 'Comment'
        m2m_table_name = db.shorten_name(u'stig_comment_sticker_list')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('comment', models.ForeignKey(orm[u'stig.comment'], null=False)),
            ('placesticker', models.ForeignKey(orm[u'stig.placesticker'], null=False))
        ))
        db.create_unique(m2m_table_name, ['comment_id', 'placesticker_id'])

        # Deleting field 'PlaceSticker.comment'
        db.delete_column(u'stig_placesticker', 'comment_id')
